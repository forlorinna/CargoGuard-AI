...
**Final engineering pass — 20 September 2026:** the fresh official evaluator run returned **1.0 / 1.0**, with unchanged metric breakdown and identical submission bytes. TypeScript, all 15 existing engine tests, 6 new document safety tests, all 14 existing API checks, 11 new document API checks, and the production build passed locally. The new document tests cover mandatory OCR review, complete evidence, wrong-document guards, session isolation, stale revisions, durable audit, and unchanged official exports. See [the actual run](evaluation/005-engineering-run.json), [score](evaluation/005-engineering-score.json), and [21-file integrity check](evaluation/005-frozen-integrity.json).

Real local browser execution extracted text from a PDF, parsed labeled Word table cells, and recognized a genuinely image-only PDF using English LSTM OCR. The scan yielded a real 95/100 recognition signal and stayed in NEEDS_REVIEW; retry preserved that state, and explicit human confirmation preserved the genuine consignee discrepancy and audit history. These tests used new synthetic fixtures, not organizer answers. See [AI/document implementation details](AI-AND-DOCUMENTS.md). Production evidence for this release is recorded separately in [DEPLOYMENT.md](DEPLOYMENT.md).

The public deployment subsequently passed all 14 existing and 11 new API checks, full baseline smoke checks, and 16 additional runtime/demo asset hashes. Browser tests confirmed PDF/Word parsing, scanned-PDF OCR at 95/100, PNG OCR at 93/100, human correction/confirmation, persistence after reload, and all seven views. Both OCR signals remained review-required until a person confirmed the case. Report-download completion in the embedded browser remains unverified and is stated separately from the verified API data.

The final tracked-source audit found no common credential-pattern matches, private evaluator files, official archive/corpus copies, dependency folders, or generated runtime/build assets. Relative Markdown links resolved and the new upstream documentation links returned HTTP 200. The only literal TODO/FIXME terms are in the historical branding audit describing its scan, not unfinished judge-facing controls. Required hosting configuration and upstream attribution remain intentionally retained.
